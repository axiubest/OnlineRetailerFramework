...
/** 扫描内容的Y值 */
#define scanContent_Y KHEIGHT * 0.15
/** 扫描内容的X值 */
#define scanContent_X KWIDTH * 0.15

/** 扫描动画线(冲击波) 的高度 */
static CGFloat animation_line_H = 5;
/** 扫描内容外部View的alpha值 */
static CGFloat scanBorderOutsideViewAlpha = 0.4;
/** 定时器和动画的时间 */
static CGFloat timer_animation_Duration = 0.05;

@interface XIU_ScanQRCodeView ()
...
@implementation XIU_ScanQRCodeView

-(instancetype)initWithFrame:(CGRect)frame WithLayer:(CALayer *)layer {
...
- (void)setupScanningQRCodeEdging {
    ...
    _animationLine.image = [UIImage imageNamed:@"QRCodeLine"];
    _animationLine.frame = CGRectMake(scanContent_X * 0.5, scanContent_layerY, self.frame.size.width - scanContent_X , animation_line_H);
    [self.basedLayer addSublayer:self.animationLine.layer];
